...
class Mqtt(object):
    def __init__(self, config:Config, devices:Dict[str,eTRVDevice], set_temperature_callback:Callable[[Dict[str,eTRVDevice], str, float],None]):
        ...
        if config.mqtt.user is not None:
            self._client.username_pw_set(config.mqtt.user, password=config.mqtt.password)
        logger.debug("connecting to {}:{}", config.mqtt.server, config.mqtt.port)
    ...
    def publish_device_data(self, name:str, data:str):
        if self._client.is_connected():
            self._client.publish(self._config.mqtt.base_topic+'/'+name+'/data', payload=data)

    def _on_connect(self, client, userdata, flags, rc):
        logger.info("Connected with result code "+str(rc))
        self._client.subscribe(self._config.mqtt.base_topic+'/+/set_temperature')
    ...
    def _on_message(self, client, userdata, msg):
        ...
        if name in self._devices.keys():
            try:
                self._set_temperature_callback(self._devices, name, float(msg.payload))
            except ValueError:
